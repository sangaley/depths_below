...
Legend: `[ ]` open · `[x]` done · `[-]` deliberately cut for after the demo.

---

## 1. Blockers — a player gets stuck, confused, or loses their save

- [ ] **Tutorial step 13 of 14 cannot be completed.** It says "use the Crew
...
- [ ] **The hull palette places the wrong block.** Seven hull items, six
...
- [ ] **Autosave writes enemy ships into the player's save.** Three unfiltered
...
- [ ] **"All crew died" never fires while any enemy lives.** The crew query is
      ...
      `ship/systems.rs:44`
- [ ] **The player can launch with zero crew.** Starter-crew spawn is suppressed
...
- [ ] **A second expedition can never reach the ending.** `discovered_logs` is
...
- [ ] **`check_poi_discovery` is the unfixed twin of the log bug.** Log
...
- [ ] **The ending plays in total silence.** `stop_flight_loops` fires on
      `OnExit(Exploring)` and `Truth` is entered from `Exploring`. `audio.rs:41`
- [ ] **The built-in starter ship has no Memory Cores.** Three were added to
...
- [ ] **Difficulty is a cliff, not a ramp.** Creature biome, density and spawn
...
- [ ] **Crossing the galaxy is free.** Max jump is 500 fuel of a 1500 tank,
...
- [ ] **Fuel burns while merely powered, not while thrusting.** Five engines at
...
- [ ] **Progression gates nothing.** `Unlocks.modules` still has no reader.
      Hull materials are gated by strings nothing ever grants, so the player is
      on Steel forever while endgame enemies are on Abyssal Alloy — a 7.5:1
      hull ratio. 52 of 157 modules are unreachable because `BuildCategory` has
      no `Structural` variant, **including ShieldEmitter and MemoryCore**.
- [ ] **Deposits are checked but never charged.** Accepting a contract is a
      free option with a downside only on death. `contracts/ui.rs:176-184`
- [ ] **Blind warp can strand you.** Landing further than `SNAP_TOLERANCE` from
...
- [ ] **~50 seconds of opening silence.** No menu audio, no station ambience.
      `interior_hum.ogg` and `machine_loop_1-3.ogg` are sitting unused.
- [ ] **Taking a hit makes no sound.** `ShipDamaged` has no handler; projectile
      impacts are explicitly unwired. Only a fully destroyed hull tile is
      audible. `combat/new_projectiles.rs:822`
- [ ] **Pausing hard-cuts the entire soundscape** and restarts it from sample
      zero on resume, because `stop_flight_loops` fires on `OnExit(Exploring)`.
- [ ] **Nothing marks launch**, the most cinematic beat in the first minute.
- [ ] **`DockingCompleted` is dead code** — a handler with no writer. Wire it
